...
//.h文件定义类  .cpp文件定义函数
class Hand; //Hand类的前向声明（未定义前，只能使用该类的指针或者引用）
...
inline uint qHash(const Card& card)//qHash函数，
{
...
public:
    ...
    void Add(const Cards& cards);//添加card元素
    void Add(const Card& card);
    void Add(const QVector<Cards>& cardsArray);

    void Remove(const Cards& cards);//删除card元素
    void Remove(const Card& card);
    void Remove(const QVector<Cards>& cardsArray);

    bool Contains(const Cards& cards) const; //该函数在Card.cpp中重载
    bool Contains(const Card& card) const;

    Cards& operator <<(const Cards& cards);//该函数在Card.cpp中重载
